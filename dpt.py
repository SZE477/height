...
huggingface_token = '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'
...
def get_depth_map(image, token=None):
    try:
        ...
        image_processor = AutoImageProcessor.from_pretrained("LiheYoung/depth-anything-small-hf", token=token)
        model = AutoModelForDepthEstimation.from_pretrained("LiheYoung/depth-anything-small-hf", token=token)
...
# Load an image
image_path = r"C:\Users\Easwar\Desktop\Height-Detection-main\Ref_image.jpg"  # Replace with the correct path to your image
...
if image is None:
    ...
    if person_detected:
        ...
        if roi:
            ...
            if closest_distance is None or farthest_distance is None:
                print("Failed to estimate distances.")
            else:
                print(f"Estimated closest distance to object: {closest_distance} meters")
                print(f"Estimated farthest distance to object: {farthest_distance} meters")
    else:
